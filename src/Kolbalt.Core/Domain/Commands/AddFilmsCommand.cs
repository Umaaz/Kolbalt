...
using Kolbalt.Core.Domain.Validations;
using NHibernate.Linq;

namespace Kolbalt.Core.Domain.Commands
{
    public class AddFilmsCommand
    {
        public AddFilmsCommand()
        {
            ValidationErrors = new Dictionary<string, IList<IDictionary<int,IList<ValidationFailure>>>>();
        }

        public IList<Film> Films { get; set; }
        public IDictionary<string, IList<IDictionary<int,IList<ValidationFailure>>>> ValidationErrors { get; set; }
        ...
        public bool Execute()
        {
            ...
            using (var tx = nhSession.BeginTransaction())
            {
                ...
                foreach (var newFilm in newFilms)
                {
                    ...
                    {
                        IList<IDictionary<int, IList<ValidationFailure>>> errors = new List<IDictionary<int,IList<ValidationFailure>>>
                                                                                       {
                                                                                           validator.RoleErrors,
                                                                                           validator.DirectorErrors,
                                                                                           validator.WriterErrors,
                                                                                           validator.GenreErrors
                                                                                       };
                        ValidationErrors.Add(newFilm.IMDBId, errors);
                        mayCommit = false;
                    }
                }
                if (mayCommit)
